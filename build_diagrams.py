# -*- coding: utf-8 -*-
"""Render all high-fidelity SuryaSetu deck diagrams (dark app-panel style)."""
import json
import numpy as np
from theme import *
...
def xray_spectrum():
    ...
    E = np.logspace(0, np.log10(200), 500)            # keV
    thermal = 1e3 * np.exp(-E/4.0) + 1                 # soft thermal
    nonthermal = 4e3 * E**(-3.0)                       # hard power-law
    ...
    ax.set_xlabel("Photon energy  (keV)", color=MUTED, fontsize=10)
    ax.set_ylabel("Flux  (a.u.)", color=MUTED, fontsize=10)
    ax.text(6.6, 700, "SoLEXS\n2–22 keV", color=SOFT, fontsize=9.5, ha="center", fontweight="bold")
    ax.text(48, 700, "HEL1OS\n8–150 keV", color=ACCENT, fontsize=9.5, ha="center", fontweight="bold")
    ax.set_title("Two payloads, two physical windows", color=ACCENT2, fontsize=13,
...
def neupert():
    ...
    hard = np.exp(-((t-9)**2)/8.0)                      # impulsive, early
    ...
    ax.plot(t, hard, color=ACCENT, lw=2.4, label="Hard X-ray  (HEL1OS)")
    ax.plot(t, soft, color=SOFT, lw=2.4, label="Soft X-ray  (SoLEXS)")
    ...
    ax.text((hpk+spk)/2, 1.12, "lead time  ~5–15 min", color=YELLOW, ha="center",
            fontsize=10, fontweight="bold")
    ax.text(9, -0.14, "hard peaks first", color=ACCENT, ha="center", fontsize=8.5)
    ax.text(spk, -0.14, "damaging soft peak", color=SOFT, ha="center", fontsize=8.5)
    ax.text(20.5, 0.30, r"HXR(t) $\approx$ d/dt SXR(t)", color=PURPLE, fontsize=11,
            fontweight="bold")
    ax.set_xlim(0, 30); ax.set_ylim(-0.02, 1.25)
    ax.set_xlabel("Time  (minutes)", color=MUTED, fontsize=10)
    ax.set_yticks([])
    ax.set_title("The Neupert effect = our forecast lead time", color=ACCENT2,
                 fontsize=13, fontweight="bold", pad=22, loc="left")
    ax.legend(loc="center right", fontsize=8.5, frameon=False, labelcolor=INK)
    ax.text(0.2, -0.30, "Illustrative (synthetic) — the physical relationship, not measured data.",
            transform=ax.transAxes, color=MUTED, fontsize=7.5, style="italic")
    fig.tight_layout()
...
def real_lightcurve():
    ...
    fig, ax = chart_fig(9.4, 2.9)
    ax.fill_between(hrs, 0, pk, color=ACCENT, alpha=0.18, label="per-block peak")
    ax.plot(hrs, avg, color=SOFT, lw=1.6, label="avg counts/s")
    ax.axhline(d["median"], color=GREEN, ls="--", lw=1.3)
    ax.text(0.4, d["median"]+3, f"quiet Sun ≈ {d['median']:.0f}/s", color=GREEN, fontsize=9)
    ax.scatter([peak_h], [d["max"]], color=RED, s=70, zorder=6)
    ax.annotate(f"real flare · {d['max']:.0f}/s @ {peak_h:.1f}h UTC",
                xy=(peak_h, d["max"]), xytext=(peak_h-7.5, d["max"]-6),
                color=RED, fontsize=9.5, fontweight="bold",
                arrowprops=dict(arrowstyle="->", color=RED, lw=1.3))
    ax.set_xlim(0, 24); ax.set_ylim(0, d["max"]*1.15)
    ax.set_xlabel("Hour of day (UTC)", color=MUTED, fontsize=9)
    ax.set_ylabel("counts / s", color=MUTED, fontsize=9)
    ax.set_xticks(range(0, 25, 3))
    ax.set_title(f"Real Aditya-L1 SoLEXS day-file  ·  {d['date']}  ·  detector {d['det']}",
                 color=ACCENT2, fontsize=11.5, fontweight="bold", pad=8, loc="left")
...
def feature_importance():
    ...
    for yi, (name, val, note) in zip(y, feats):
        ...
        ax.text(val-0.02, yi, f"{val:.2f}", va="center", ha="right", color="#10131c",
                fontsize=8.6, family=MONO, fontweight="bold")
        ax.text(1.02, yi, note, va="center", color=MUTED, fontsize=7.8)
    ax.set_xlim(0, 1.0); ax.set_ylim(-0.6, len(feats)-0.4)
    ax.set_yticks(y); ax.set_yticklabels([f[0] for f in feats], fontsize=8.8, family=MONO, color=INK)
    ax.set_xticks([0, 0.5, 1.0])
    ax.set_xlabel("relative importance (SHAP)", color=MUTED, fontsize=9)
...
def tss():
    ...
    thr = 0.55
    # classes
    flares_p = rng.uniform(0.45, 0.97, 26)     # actual flares (high prob mostly)
    quiet_p  = rng.uniform(0.02, 0.62, 70)     # actual quiet
    for p in flares_p:
        c = GREEN if p >= thr else YELLOW
        ax.scatter(p, rng.uniform(0.55, 0.95), color=c, s=26, alpha=0.9)
    for p in quiet_p:
        c = RED if p >= thr else "#3a4f7a"
        ax.scatter(p, rng.uniform(0.05, 0.45), color=c, s=22, alpha=0.85)
    ax.axvline(thr, color=INK, ls="--", lw=1.3)
    ax.text(thr+0.01, 0.99, "threshold (TSS-tuned)", color=INK, fontsize=8, va="top")
    ax.set_xlim(0, 1); ax.set_ylim(0, 1)
    ax.set_yticks([]); ax.set_xlabel("forecast probability  P(flare ≤ N min)", color=MUTED, fontsize=9)
    ax.set_title("Evaluated on TSS, not accuracy", color=ACCENT2, fontsize=12.5,
                 fontweight="bold", pad=10, loc="left")
    ...
    ax.legend(handles=leg, loc="lower right", fontsize=7.6, frameon=False, labelcolor=INK, ncol=2)
    ax.text(0.02, -0.30, "TSS = TPR − FPR    ·  +1 perfect, 0 = no skill.  A constant 'no-flare' forecast scores 0.",
            transform=ax.transAxes, color=PURPLE, fontsize=8.5, fontweight="bold")
    fig.tight_layout()
...
def process_flow():
    ...
    title(ax, "End-to-end data flow", y=94, size=15)
    box(ax, 3, 73, 19, 14, "Aditya-L1 payloads\nSoLEXS · HEL1OS\nraw FITS day-files", ec=ACCENT, bold=True, fs=9.5)
    box(ax, 26, 73, 16, 14, "Ingest + GTI\nmask gaps /\nocculations", fs=9.5)
    box(ax, 46, 73, 16, 14, "Fuse + resample\nSDD1+2, soft+hard\n→ 1-second grid", fs=9.5)
    box(ax, 66, 73, 17, 14, "Clean fused\ndual-channel\ntime series", ec=ACCENT2, bold=True, fs=9.5)
    ...
    box(ax, 7, 35, 36, 16, "CLASSICAL NOWCAST\nadaptive baseline + rise-rate + hysteresis\nonset / peak / end · relative A→X class",
        fc="#10140e", ec=GREEN, fs=9.2)
    box(ax, 57, 35, 36, 16, "ML FORECAST\nLightGBM (physics features) + MiniROCKET (raw)\n→ calibrated  P(flare ≤ N min)",
        fc="#0e1320", ec=SOFT, fs=9.2)
    box(ax, 17, 15, 26, 12, "Master flare catalogue\nonset·peak·end · class · SNR", ec=GREEN, fs=9.2, bold=True)
    box(ax, 57, 15, 26, 12, "Calibrated alerts\nlead-time + confidence", ec=ACCENT, fs=9.2, bold=True)
    ...
    box(ax, 36, 1.5, 28, 11, "Operator dashboard (Streamlit)\nGREEN / AMBER / RED  ·  replay",
        fc=PANEL2, ec=ACCENT2, fs=9.5, bold=True)
...
def usecase():
    ...
    title(ax, "Who uses it", y=94, size=14)
    # actors
    def actor(cx, cy, label, color):
        ...
        ax.add_line(Line2D([cx, cx+3],[cy-7, cy-11], color=color, lw=2))
        ax.text(cx, cy-15, label, ha="center", color=INK, fontsize=9.5, fontweight="bold")
    actor(16, 80, "Ops\noperator", ACCENT)
    ...
    for y, txt, col, ax0 in cases:
        box(ax, 34, y-3.4, 62, 6.8, txt, ec=col, fs=8.6, round=3.0, align="left")
...
def dashboard():
    ...
    ax.text(10, 92.2, "ALERT  —  M-class flare forecast  ·  ~7 min lead  ·  confidence 0.82",
            va="center", color=RED, fontsize=11, fontweight="bold")
    # dual light curve
    box(ax, 3, 50, 60, 35, "", fc=PANEL, ec=LINE)
    ax.text(6, 81, "Dual light curve — soft + hard + playhead", color=INK, fontsize=9.5, fontweight="bold")
    t = np.linspace(0,1,200)
    soft = 0.46*np.exp(-((t-0.62)**2)/0.012)+0.04
    hard = 0.62*np.exp(-((t-0.54)**2)/0.0022)+0.03
    ax.plot(6+t*54, 54+soft*24, color=SOFT, lw=2.2)
    ax.plot(6+t*54, 54+hard*24, color=ACCENT, lw=1.9)
    ax.add_line(Line2D([6+0.6*54, 6+0.6*54], [51, 84], color=INK, lw=1.2, ls="--", zorder=4))
    ax.text(6+0.6*54+0.6, 78, "playhead", color=MUTED, fontsize=7.5)
    ax.legend(handles=[Line2D([],[],color=SOFT,label="soft (SoLEXS)"),
                       Line2D([],[],color=ACCENT,label="hard (HEL1OS)")],
              loc="upper right", fontsize=7.4, frameon=False, labelcolor=INK,
              bbox_to_anchor=(0.62, 0.86))
    # hard/soft ratio
    box(ax, 66, 68, 31, 17, "", fc=PANEL, ec=LINE)
    ax.text(68.5, 81.5, "Hard / soft ratio (precursor) ↑", color=PURPLE, fontsize=8.6, fontweight="bold")
    sig = 1/(1+np.exp(-(t-0.55)*12))
    ax.plot(68.5+t*26, 70+sig*9, color=PURPLE, lw=2.2)
    # gauge
    box(ax, 66, 50, 31, 16, "", fc=PANEL, ec=ACCENT)
    ax.text(81.5, 63.5, "P(flare ≤ N min)", color=INK, fontsize=8.6, fontweight="bold", ha="center")
    ...
    ax.text(cx, cy-0.5, "0.82", ha="center", va="center", color=RED, fontsize=18,
            fontweight="bold", family=MONO)
    # catalogue table
    box(ax, 3, 4, 94, 42, "", fc=PANEL, ec=LINE)
    ax.text(6, 42, "Flare catalogue", color=INK, fontsize=9.5, fontweight="bold")
    cols = ["onset","peak","end","class","SNR","lead"]
    xs = [6, 22, 38, 54, 66, 78]
    for x,c in zip(xs, cols):
        ax.text(x, 36, c, color=ACCENT2, fontsize=8.4, fontweight="bold", family=MONO)
    ...
    for i,r in enumerate(rows):
        yy = 31-i*6
        for x,v in zip(xs, r):
            ax.text(x, yy, v, color=MUTED, fontsize=8, family=MONO)
...
def architecture():
    ...
    title(ax, "Layered system architecture", y=98.5, size=15)
    def layer(y, h, label, color):
        box(ax, 1.5, y, 97, h, "", fc="#0c1120", ec=color, round=1.2, lw=1.4)
        ax.text(3.4, y+h-3.4, label, color=color, fontsize=10.5, fontweight="bold")
    layer(78, 13, "DATA  ·  free & public", ACCENT)
    box(ax, 7, 79.3, 26, 7.6, "SoLEXS\nsoft 2–22 keV · .lc/.gti", fs=8.4, ec=ACCENT)
    box(ax, 37, 79.3, 26, 7.6, "HEL1OS\nhard 8–150 keV · .lc", fs=8.4, ec=ACCENT)
    box(ax, 67, 79.3, 26, 7.6, "GOES catalog\nlabels (A→X) · CSV", fs=8.4, ec=ACCENT2)
    layer(61, 16, "INGEST · PREPROCESS · LABEL", ACCENT2)
    box(ax, 7, 63, 25, 9, "GTI mask\n+ astropy FITS", fs=8.4)
    box(ax, 37, 63, 25, 9, "Fuse SDD1/2 + soft/hard\n→ 1-s grid (pandas)", fs=8.4, ec=ACCENT2)
    box(ax, 67, 63, 26, 9, "GOES cross-ref\n→ flare/quiet windows", fs=8.4)
    layer(36, 22, "PROCESSING  ·  classical where classical wins, ML where ML wins", SOFT)
    box(ax, 6, 39, 28, 13, "NOWCAST (classical)\nadaptive baseline + rise-rate\n→ master catalogue", fc="#10140e", ec=GREEN, fs=8.4)
    box(ax, 37, 44, 27, 7.5, "LightGBM · physics features", fc="#0e1320", ec=SOFT, fs=8.2)
    box(ax, 37, 38.5, 27, 5, "MiniROCKET · raw window", fc="#0e1320", ec=SOFT, fs=8.2)
    box(ax, 67, 39, 26, 13, "Calibrated fusion (isotonic)\n→ P(flare ≤ N min)\n+ lead-time", fc="#0e1320", ec=PURPLE, fs=8.4)
    arrow(ax,(64,45),(67,45),color=PURPLE)
    layer(18, 15, "EVALUATE · honest metrics", MUTED)
    box(ax, 7, 20, 40, 8, "TSS (primary) · HSS · ROC-AUC · Brier", fs=8.4, ec=ACCENT2)
    box(ax, 52, 20, 41, 8, "reliability diagram · lead-time dist · time-blocked CV", fs=8.4)
    layer(2, 13, "DELIVERY", ACCENT)
    box(ax, 28, 3.5, 44, 7, "Streamlit dashboard · alerts · replay · SHAP 'why'",
        fc=PANEL2, ec=ACCENT, fs=9, bold=True)
...
def tech_stack():
    rows = [
        ...
        ("Forecaster A",    "LightGBM",           "fast, interpretable, sample-efficient on rare events","MIT",  SOFT),
        ("Forecaster B",    "MiniROCKET (sktime)","CPU, deterministic, SOTA-ish on raw time series",  "BSD",    SOFT),
        ("Calibrate/metric","scikit-learn",       "isotonic calibration · TSS/HSS/ROC/Brier · blocked CV","BSD", SOFT),
        ...
        ("Interface",       "Streamlit",          "fastest path to dual curves + alerts + replay",    "Apache", GREEN),
        ("Environment",     "Python 3.11 (uv)",   "prebuilt wheels; avoids the 3.14 astropy build break","PSF", MUTED),
    ]
    fig, ax = panel_fig(9.6, 5.4)
    title(ax, "Technology stack — every choice has a reason", y=96, size=14)
    # header
    hx = [4, 22, 44, 88]
    heads = ["LAYER", "TOOL", "WHY THIS TOOL", "LICENSE"]
    ax.text(hx[0], 88, heads[0], color=MUTED, fontsize=8.6, fontweight="bold")
    ax.text(hx[1], 88, heads[1], color=MUTED, fontsize=8.6, fontweight="bold")
    ax.text(hx[2], 88, heads[2], color=MUTED, fontsize=8.6, fontweight="bold")
    ax.text(hx[3], 88, heads[3], color=MUTED, fontsize=8.6, fontweight="bold")
    ax.add_line(Line2D([3,96],[86,86], color=LINE, lw=1))
    y = 81; dy = 7.7
    for layer, tool, why, lic, col in rows:
        box(ax, 3, y-2.6, 1.1, 5.2, "", fc=col, ec=col, round=0.4)   # color tab
        ax.text(hx[0]+1.2, y, layer, color=INK, fontsize=8.8, va="center")
        ax.text(hx[1], y, tool, color=col, fontsize=8.8, va="center", fontweight="bold", family=MONO)
        ax.text(hx[2], y, why, color=MUTED, fontsize=8.2, va="center")
        ax.text(hx[3], y, lic, color=INK, fontsize=8.2, va="center", family=MONO)
        ax.add_line(Line2D([3,96],[y-3.9,y-3.9], color="#14203a", lw=0.6))
        y -= dy
    save(fig, out("tech_stack.png"))

# ============================================================ 11 · DATA FUNNEL
def data_funnel():
    fig, ax = panel_fig(5.2, 4.3)
    title(ax, "Smart data sampling", y=94, size=14)
    # three trapezoids narrowing
    def trap(y, topw, botw, color, label, sub):
        ...
        ax.text(cx, y+5.2, label, ha="center", color=INK, fontsize=11, fontweight="bold")
        ax.text(cx, y+1.6, sub, ha="center", color=MUTED, fontsize=8)
    ...
    ax.text(50, 7, "We never brute-force 70 GB —\nlabels point us to the 7% that matters.",
            ha="center", color=PURPLE, fontsize=8.6, fontweight="bold")
    save(fig, out("data_funnel.png"))

# ============================================================ 12 · FEATURE GRID
def features_grid():
    fig, ax = panel_fig(12.5, 4.3)
    feats = [
        ("01", "Automated nowcast catalogue", "real-time soft+hard detection merged into one\nmaster catalogue · onset/peak/end · class · SNR", ACCENT),
        ("02", "Calibrated forecast + lead time", "P(flare ≤ N min) with calibrated confidence\nand a measured lead-time distribution", SOFT),
        ("03", "Live dashboard + visual alerts", "dual curves · hard/soft ratio · gauge ·\ngreen/amber/red banner that flips before the peak", GREEN),
        ("04", "Full A→X dynamic range", "adaptive baseline + dual SoLEXS detectors\ncatch faint A/B/C and giant M/X flares", ACCENT2),
        ("05", "Explainable alerts (SHAP)", "every forecast states which precursor fired it —\nscientists can trust and audit it", PURPLE),
        ("06", "Offline & reproducible", "no cloud, no cost, no proprietary software —\nend-to-end on one laptop from public data", RED),
    ]
    W, H = 30.5, 34; gx, gy = 2.5, 3.0
    xs = [2.5, 35, 67.5]; ys = [57, 11]
    i = 0
    for r in range(2):
        for c in range(3):
            num, ttl, body, col = feats[i]; i += 1
            x, y = xs[c], ys[r]
            box(ax, x, y, W, H, "", fc=PANEL, ec=LINE, round=1.4)
            box(ax, x+2, y+H-9, 8, 6.2, num, fc="#0c1120", ec=col, tc=col, fs=12, bold=True, family=MONO)
            ax.text(x+2, y+H-14, ttl, color=INK, fontsize=9.6, fontweight="bold", va="top")
            ax.text(x+2, y+H-19, body, color=MUTED, fontsize=7.8, va="top", linespacing=1.5)
    save(fig, out("features_grid.png"))

if __name__ == "__main__":
    xray_spectrum(); neupert(); real_lightcurve(); feature_importance(); tss()
    process_flow(); usecase(); dashboard(); architecture(); tech_stack()
    data_funnel(); features_grid()
